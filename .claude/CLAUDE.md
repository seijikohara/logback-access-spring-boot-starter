...
- A CycloneDX SBOM is generated per published module by the CycloneDX Gradle plugin and attached to each GitHub Release. The SBOMs are not published as part of the Maven Central artifacts.
...
| `codeql.yml` | push / pull request / scheduled | CodeQL security analysis. |
| `docs.yml` | push to `main` (docs paths) / manual dispatch | Build and deploy the VitePress documentation site. |
| `dependency-submission.yml` | push to `main` | Submit the Gradle dependency graph to GitHub's Dependency Submission API. |
